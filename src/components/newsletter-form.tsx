...
export function NewsletterForm() {
    ...
    if (status === "success") {
        return (
            <div role="status" aria-live="polite" className="flex items-center gap-2.5 py-3 animate-in fade-in duration-500 text-green-700 dark:text-green-400">
                <FontAwesomeIcon icon={faCheckCircle} aria-hidden="true" className="h-5 w-5" />
    ...
    return (
        <form onSubmit={handleSubmit} noValidate className="w-full">
            <div className="flex gap-2 flex-col sm:flex-row">
                ...
                />
                {/* Honeypot */}
                <div inert aria-hidden="true" className="absolute -left-[9999px] h-px w-px overflow-hidden">
                    <input
                        type="text"
                        name="website"
                        ref={hpRef}
                ...
                <button
                    ...
                    className="px-5 py-2.5 text-sm font-medium rounded-full transition-[background-color,box-shadow,opacity,transform] disabled:opacity-60 shrink-0 flex items-center justify-center gap-2 group bg-[#F43600] hover:bg-[#FF4D1A] text-white shadow-[0_2px_12px_rgba(244,54,0,0.15)] hover:shadow-[0_4px_20px_rgba(244,54,0,0.25)] dark:shadow-[0_4px_24px_rgba(244,54,0,0.25)] dark:hover:shadow-[0_6px_32px_rgba(244,54,0,0.35)]"
                >
                    {status === "loading" ? (
